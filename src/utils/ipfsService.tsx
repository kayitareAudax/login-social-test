...
import { IPFS_METADATA_API_URL, API_URL, LS_KEY } from '../config'
...
// IPFS_METADATA_API_URL
export const reduceTotalSupply = async (_data) => {
    const resFile = await axios.post<PostResponse>(
        `${IPFS_METADATA_API_URL}/reduce-totalsupply`,
        _data,
        {
            headers: {
                'Content-Type': 'application/json',
...
export const uploadImageToServer = async (image) => {
    const accessToken = JSON.parse(localStorage.getItem(LS_KEY))
